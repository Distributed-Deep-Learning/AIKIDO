...
func (kf *Kungfu) ReshapeStrategy(reshapeOn int) (bool, error) {
	...
	log.Debugf("calling reshape method here")
	...
	if !kf.DelayOn && reshapeOn == 0 {
		...
		log.Debugf("delay on reshape off")
...
func parseDelayConfigFile() map[int]Delay {
	// pwd, _ := os.Getwd()
	data, err := ioutil.ReadFile("/home/gcp_ghobadi_google_mit_edu/src/KungFu/generated_configs/test_75ms.config")
	if err != nil {
